...
func fetchAllResults() ([]SpeedTestResult, error) {
	...
	for rows.Next() {
		...
		var timestamp time.Time // Using `time.Time` to fetch and then format the timestamp
...
// apiHandler serves as the HTTP handler to return all results in JSON format.
func apiHandler(w http.ResponseWriter, r *http.Request) {
	results, err := fetchAllResults()
	if err != nil {
