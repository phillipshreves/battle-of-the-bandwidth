            ...
            <div className="grid grid-cols-1 gap-2">
                {commonSchedules.map(({ label, expression }) => (
                    <button
                        ...
                        className="text-left px-3 py-2 text-sm bg-background/60 hover:bg-background/80 rounded-md transition-colors"
                    >
                        <div className="font-medium text-foreground">{label}</div>
                        <div className="text-muted text-xs">{expression}</div>
                    </button>
                ))}
            </div>
